...
Factory.prototype.clean = function(cb) {
  var passwordOption = this.conf.password ? ' -p' + this.conf.password : '';
  var commands = [
    'mysql -u ' + this.conf.user + passwordOption + ' -e "drop database if exists \\`' + this.conf.database + '\\`"',
    'mysql -u ' + this.conf.user + passwordOption + ' -e "create database \\`' + this.conf.database + '\\` default character set utf8 collate utf8_general_ci"',
    'mysql -u ' + this.conf.user + passwordOption + ' "' + this.conf.database + '" < ' + this.conf.dbStructureFile
  ];
