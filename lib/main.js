...
Factory.prototype.clean = function(cb) {
  var mysqlConnOptions = [ 'mysql' ];
  if (this.conf.user) { mysqlConnOptions.push('--user ' + this.conf.user); }
  if (this.conf.host) { mysqlConnOptions.push('--host ' + this.conf.host); }
  if (this.conf.port) { mysqlConnOptions.push('--port ' + this.conf.port); }
  if (this.conf.password) { mysqlConnOptions.push('-p' + this.conf.password); }

  var mysqlClientConnection = mysqlConnOptions.join(' ');

  var commands = [
    mysqlClientConnection + ' -e "drop database if exists \\`' + this.conf.database + '\\`"',
    mysqlClientConnection + ' -e "create database \\`' + this.conf.database + '\\` default character set utf8 collate utf8_general_ci"',
    mysqlClientConnection + ' "' + this.conf.database + '" < ' + this.conf.dbStructureFile
  ];
